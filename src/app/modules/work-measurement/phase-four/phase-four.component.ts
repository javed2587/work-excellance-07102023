...
export class PhaseFourComponent implements OnInit {
  ...
  fetchColor(val: any) {
    ...
    this.modal = false;
    this.showLightModal = false;
    this.showCardYellow = false;

    if (val.background == "#00FF00") {
      this.showCardGreen = true;
    } else if (val.background == '#FFFF00') {
      this.showCardYellow = true;
    } else if (val.background == '#FF0000') {
      this.showCardRed = true;
    } else {
      this.showCardBlue = true;
    }
   console.log('val.background: ',val.background)
   console.log('this.graphColor: ',this.graphColor)
  }
  addGraphValueModal() {
